...
class Cart(models.Model):
    buyer = models.ForeignKey(
        User, on_delete=models.CASCADE, default=None, related_name="buyer")
    item = models.ForeignKey(
        Product, on_delete=models.CASCADE, default=None, related_name="item")
...
class Order(models.Model):
    ...
    shipping_fee = models.DecimalField(
        default=8.00, max_digits=1000, decimal_places=2)
    email = models.EmailField(
        max_length=111, blank=True, help_text="enter your active email")
    address = models.CharField(
        max_length=400, blank=True, help_text="house no. or P.O.Box")
    city = models.CharField(max_length=111, blank=True)
    state = models.CharField(max_length=111, blank=True,
                             help_text="province, country.")
    ...
    def get_subtotal(self):
        subt = [x.get_cart_total for x in self.orderitems.all()]
        return sum(subt)
